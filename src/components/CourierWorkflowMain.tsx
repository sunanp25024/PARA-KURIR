...
import { Package, Scan, Truck, AlertTriangle, TrendingUp, CheckCircle, RotateCcw, ArrowRight, Clock } from 'lucide-react';
...
const CourierWorkflowMain = () => {
  ...
  const getStepIcon = (step: string, status: string) => {
    ...
    if (status === 'active') {
      switch (step) {
        ...
        case 'scan':
          return <Scan className={`${iconClass} text-blue-600`} />;
        case 'delivery':
    ...
    switch (step) {
      ...
      case 'scan':
        return <Scan className={`${iconClass} text-gray-400`} />;
      case 'delivery':
        return <Truck className={`${iconClass} text-gray-400`} />;
      case 'pending':
        return <Clock className={`${iconClass} text-gray-400`} />;
      case 'performance':
        return <TrendingUp className={`${iconClass} text-gray-400`} />;
      default:
        return null;
    }
  };
        ...
        return <div className="space-y-6">
            ...
            <Card className="border-blue-200 bg-blue-50">
              <CardContent className="pt-6">
                <Button onClick={handleStartNewDay} className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700" size="lg">
  ...
  const steps = [{
    key: 'input',
    label: 'Input Paket',
    count: dailyPackages.length
  }, {
    key: 'scan',
    label: 'Scan & Kelola',
    count: deliveryPackages.length
  }, {
    key: 'delivery',
    label: 'Pengantaran',
    count: `${deliveredPackages.length}/${deliveryPackages.length}`
  }, {
    key: 'pending',
  ...
  return <div className="space-y-6 max-w-4xl mx-auto p-6">
